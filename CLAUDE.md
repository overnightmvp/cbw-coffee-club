...
# - PAYLOAD_SECRET (32-char random string for CMS)
# - DATABASE_URI (Supabase PostgreSQL connection string with ?schema=payload)
# - BLOB_READ_WRITE_TOKEN (Vercel Blob for blog images)
...
**Payload CMS (Blog Management):**
- Access at `/admin-cms` (separate from admin portal)
- Manage blog posts, media uploads, content scheduling
- Uses separate `payload` schema in PostgreSQL to avoid table conflicts
...
- **Payload CMS 3.75** for blog management (separate `payload` schema)
...
**CMS Portal:**
- `/admin-cms` - Payload CMS admin interface
- Manages: Posts (blog), Media (images), Users (CMS admins)
- API routes under `/api/payload/*` (auto-generated by Payload)
...
**Payload CMS Schema (Separate `payload` schema):**
```sql
-- Auto-generated by Payload CMS
posts (id, title, slug, content, status, publishedAt, category, conversionGoal,
       targetKeywords[], internalLinks[], relatedPosts[], featured_image)
media (id, filename, mimeType, filesize, url, alt)
users (id, email, role, createdAt)  -- CMS admin users, not end users
```
...
- `src/payload.config.ts` - Payload CMS configuration (PostgreSQL adapter, collections)
- `src/collections/Posts.ts`, `Media.ts`, `Users.ts` - Payload collections
...
- **CMS Portal**: Separate Payload authentication (uses `users` collection in `payload` schema)
...
- **Payload CMS** integrated for blog management (`/admin-cms`)
- Blog posts stored in `payload.posts` table (separate from main Supabase schema)
- Content categories: Event-focused, Coffee education, Venue spotlights, Location guides
- 30-post Chadstone-area content blitz planned (see `.planning/phases/01-Chadstone-Deep-Dive/`)
- Internal linking strategy: Posts link to vendor profiles, related posts
...
- Payload CMS uses separate PostgreSQL schema (`payload`) to avoid table conflicts with main app
